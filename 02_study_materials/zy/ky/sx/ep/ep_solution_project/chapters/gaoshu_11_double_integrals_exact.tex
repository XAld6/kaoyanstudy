...
\[
\int_0^1dy\int_y^1
\left(\frac{e^y}{1+x^2}+\frac{e^y}{1+y^2}\right)dx.
\]
\begin{enumerate}[label=\Alph*., leftmargin=2.2em]
...
\end{problemblock}
\begin{solutionblock}
\analysis{先严格按原图题面计算。原图中第二项也是 \(e^y/(1+y^2)\)，若把它误看或校正为 \(e^x/(1+y^2)\)，答案会变成 A。}
严格按原图题面，积分区域为
\[
0\le y\le x\le1.
\]
...
\[
\int_0^1e^y\left[
\int_y^1\frac{dx}{1+x^2}
+\int_y^1\frac{dx}{1+y^2}
\right]dy.
\]
即
\[
\int_0^1e^y\left[
\frac\pi4-\arctan y+\frac{1-y}{1+y^2}
\right]dy.
\]
对第一项分部积分：
\[
\int_0^1e^y\left(\frac\pi4-\arctan y\right)dy
=-\frac\pi4+\int_0^1\frac{e^y}{1+y^2}\,dy.
\]
所以原题面对应的真实积分值为
\[
I=-\frac\pi4+\int_0^1\frac{(2-y)e^y}{1+y^2}\,dy
\approx1.093560681.
\]
而四个选项的数值分别约为
\[
1.3495,\quad 2.1349,\quad 2.6991,\quad 4.2699,
\]
均不等于该值。因此严格按原印刷题面，四个选项无正确项。
...
\]
被积函数变为 \(h(x,y)+h(y,x)\)。三角形
\[
0\le y\le x\le1
\]
与其关于直线 \(y=x\) 的对称三角形合成单位正方形，于是校正后的积分为
\[
\iint_{[0,1]^2}\frac{e^y}{1+x^2}\,dxdy
=\int_0^1e^y\,dy\int_0^1\frac{dx}{1+x^2}
=\frac{\pi(e-1)}4.
\]
\answer{严格按原印刷选项：无正确项；若将第二项校正为 \(\dfrac{e^x}{1+y^2}\)，则选 A。}
\examnote{对称扩展法要求被积函数确为 \(h(x,y)+h(y,x)\)；本题原图第二项指数会改变答案，必须先核准题面。}
\end{solutionblock}
...
\]
若改为先定 \(r\)，则 \(1\le r\le2\)，且
\[
\theta\le \arccos\frac1r,\qquad
\theta\le \arccos\frac r2.
\]
两者在 \(r=\sqrt2\) 处分界，因此
\[
\int_1^{\sqrt2}r\,dr\int_0^{\arccos(1/r)}
f(r\cos\theta,r\sin\theta)\,d\theta
+\int_{\sqrt2}^{2}r\,dr\int_0^{\arccos(r/2)}
f(r\cos\theta,r\sin\theta)\,d\theta,
\]
这正是原题选项 C。
\answer{原题选项 C；等价地，\(\displaystyle \int_0^{\pi/4}\int_{\sec\theta}^{2\cos\theta} f(r\cos\theta,r\sin\theta)r\,dr\,d\theta\)}

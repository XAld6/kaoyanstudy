...
\end{problemblock}
\begin{solutionblock}
\analysis{原题写 \(f\) 是 \([0,1]\) 上正值连续函数，同时又给 \(f(0)=0\)。严格字面会矛盾；结合结论和证明需要，应理解为 \(f(x)>0\) 在 \((0,1]\) 上成立，且端点 \(f(0)=0\)。先对 \(\int_0^x f^3\) 与 \(\left(\int_0^x f\right)^2\) 用 Cauchy 中值定理，再对 \(f^2\) 与 \(\int_0^x f\) 用一次 Cauchy 中值定理。}
令
\[
...
=\frac{f(\xi)}{\xi-a}.
\]
将 \(f(\xi)=f'(\eta)(\xi-a)\) 代入 Cauchy 中值定理所得等式，得
\[
2\xi\int_a^b f(x)\,dx
=(b^2-a^2)f'(\eta)(\xi-a).
\]

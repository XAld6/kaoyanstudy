...
因此 \(B\) 与 \(C\) 合同，D 项正确。}
\answer{D}
\examnote{合同的定义是存在可逆矩阵 \(P\) 使 \(C=P^TBP\)。对非对称矩阵不能只套二次型惯性指数，最稳妥的方法是构造或验证合同变换矩阵。}
\end{solutionblock}
...
\item 若 \(\gamma=(1,0,4)^T\)，求 \(A^{2024}\gamma\)。
\end{enumerate}
\end{problemblock}
\begin{solutionblock}
\analysis{设
...
\[
A^{2024}\gamma=2^{2024}(2,-1,3)^T,\qquad \gamma=(1,0,4)^T.
\]}

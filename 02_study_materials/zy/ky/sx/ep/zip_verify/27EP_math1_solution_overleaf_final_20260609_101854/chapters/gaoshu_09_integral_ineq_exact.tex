...
2f(0)+f(1)>0.
\]
原 PDF 中 A 与 C 完全相同，B 与 D 完全相同。严格按原印刷选项看，A、C 均为正确项；若按单选题去重理解，正确结论是
\[
f\left(\frac13\right)<0,\qquad 2f(0)+f(1)>0.
\]
\answer{严格按原印刷选项：A、C；按去重后的命题意图：A/C 对应的结论。}
...
右端是区间平均值，交换 \(a,b\) 后数值不变，故不妨设 \(a<b\)。

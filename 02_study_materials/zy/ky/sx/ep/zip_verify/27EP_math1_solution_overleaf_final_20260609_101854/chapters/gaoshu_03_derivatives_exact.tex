...
\end{problemblock}
\begin{solutionblock}
\analysis{严格地说，原题把任意常数幂 \(x^\alpha,x^\beta\) 放在 \([-1,1]\) 上，负半轴的实值定义需要额外约定；本题按考研常见处理，考查 \(x\to0\) 时的幂阶与振荡强弱。振荡来自 \(\sin x^\beta\)，因为 \(\beta<0\)，当 \(x\to0\) 时内层趋于无穷。先看可导性，再看导函数中的最坏项。}
由导数定义，
\[
...
\]
存在性与 \(f'(0)\) 存在等价。
A 只对应 \(1-\cos h\ge0\)，主要检测右侧单侧；C 中 \(h-\sinh h\sim-h^3/6\)，分母却是 \(h^2\)，不能等价为导数；D 与 \(x\to0\) 无关。
\answer{B。}
\method{方法二（反例排除）}
A 可取右导数存在但左右导数不同的函数排除；C 可取 \(f(t)=|t|^{2/3}\) 一类使选项极限存在但导数不存在；D 只约束无穷远处，不影响 0 点可导性。
...
\end{problemblock}
\begin{solutionblock}
\analysis{导函数在 \(x_0\) 两侧分别有有限极限，只能控制每一侧内部的振荡，使左右单侧极限各自存在；但它不能保证左右单侧极限相等，也不能控制 \(f(x_0)\) 的取值。}
对 \(x\) 与 \(y\) 位于 \(x_0\) 同侧并趋于 \(x_0\)，由中值定理
\[
\frac{f(x)-f(y)}{x-y}=f'(\xi)\to a.
\]
这说明 \(f(x)\) 在 \(x\to x_0^+\) 和 \(x\to x_0^-\) 时分别有有限单侧极限，但两侧极限未必相等。例如取 \(x_0=0\)，
\[
f(x)=
\begin{cases}
0,&x<0,\\
...
\end{cases}
\]
则 \(f'(x)=0\ (x\ne0)\)，左右导数极限都等于 \(0\)，但 \(\lim_{x\to0}f(x)\) 不存在。因此 C 不一定成立，A、B 更不成立。
\answer{D。}
\examnote{去心邻域的导数信息不能自动拼合左右两侧的函数值；左右极限是否相等、中心点是否连续，都要另行检查。}
\end{solutionblock}

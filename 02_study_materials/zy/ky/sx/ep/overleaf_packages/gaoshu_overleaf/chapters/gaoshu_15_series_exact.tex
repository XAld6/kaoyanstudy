...
③ 不正确。正项递减且可和，并不保证相邻比值有极限。可以构造分段常数或近似分段常数的正项可和数列，使区间内部比值接近 \(1\)，区间交界处比值接近某个小于 \(1\) 的数，从而比值极限不存在。

④ 不正确。正项级数收敛只推出 \(a_n\to0\)，不推出最终单调。例如可令偶数项偶尔略大于前一奇数项，但总体仍受 \(\sum n^{-2}\) 控制，正项级数仍收敛。
...
③ 不正确。不等式 \(a_n\le b_n\) 是普通大小关系，不是绝对值控制。即使 \(r<R\)，也不能推出边界 \(x=r\) 处 \(\sum(a_n-b_n)r^n\) 绝对收敛。

④ 不正确。同样，普通夹逼 \(a_n\le c_n\le b_n\) 不能控制 \(|c_n|\) 的指数增长，也可能出现抵消或符号变化，不能推出半径介于 \(r\) 与 \(R\) 之间。
...
=\frac{(-1)^n}{\sqrt n}+O\left(\frac1n\right)+\frac1n,
\]
可见除交错的 \(1/\sqrt n\) 主项外，还含有正的调和级数型项。更直接地，把偶项和后继奇项配对，可得每一对约为 \(1/n\)，故级数发散。
...
B 错。绝对收敛不保证项的绝对值最终单调，哪怕比值极限存在，也可以构造带小幅振荡的 \(n^{-2}\) 型项。
...
D 错。条件收敛不保证绝对值最终单调，例如可在 \((-1)^n/n\) 的基础上加入趋零的小振荡因子，使比值仍趋于 \(-1\)，但 \(|u_n|\) 无最终单调性。
}
\answer{C}

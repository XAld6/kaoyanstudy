...
\]
严格地说，左端在 \(n=1\) 时为等号；当 \(n\ge2\) 时为严格不等式。也可用归纳法证明左端：
\[
P_1=\frac12,\qquad
P_{n+1}=P_n\frac{2n+1}{2n+2},
\]
若 \(P_n\ge\frac1{2n}\)，则
\[
P_{n+1}\ge\frac{2n+1}{2n(2n+2)}>\frac1{2n+2}\quad(n\ge1).
\]
右边等价于
\[
P_n^2<\frac1{2n+1},
...
\begin{solutionblock}
\analysis{原图中本题下标有一处不自洽：题干括号和第③项疑似出现 \(n=0\)、\(k=0\)，但第①项含 \(\frac1{2^n-1}\)，在 \(n=0\) 时无意义。因此下面按本题自洽版本理解为 \(n\ge1\)，且第③项为 \(\sum_{k=1}^n x_k\)。

这题的关键是看出给定增量小于一个可望远镜的差：
\[
...
\answer{C}
\examnote{数列不等式题常把右侧增量改写成“某个辅助项的差”，从而构造单调量。若严格按原图的 \(k=0\) 且不补充 \(x_0\) 条件，则第③项无法判定，应视为原题下标印刷问题。}
\end{solutionblock}

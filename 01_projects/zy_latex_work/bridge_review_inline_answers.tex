...
\hypersetup{colorlinks=true,linkcolor=branddark,urlcolor=branddark,pdfauthor={Codex},pdftitle={桥梁工程期末复习题（题后答案版）}}
...
\pagecolor{paper}
\vspace*{22mm}
{\sffamily\bfseries\fontsize{30}{36}\selectfont\color{branddark} 桥梁工程期末复习题（题后答案版）\par}
\vspace{5mm}
...
\end{choices}
\begin{answerbox}
\textbf{答案：} C. 桥面，在纵向相邻两梁端之间或梁端与桥台之间
\end{answerbox}
\end{questionbox}
...
\end{choices}
\begin{answerbox}
\textbf{答案：} C. 公路—Ⅰ级和公路—Ⅱ级汽车荷载采用相同的车辆荷载标准值
\end{answerbox}
\end{questionbox}
...
\end{choices}
\begin{answerbox}
\textbf{答案：} A. 只有在结构上可能同时出现的作用，才进行组合；；D. 多个偶然作用不同时参与组合。
\end{answerbox}
\end{questionbox}
...
\end{choices}
\begin{answerbox}
\textbf{答案：} C. 在竖向荷载作用下，梁式桥无水平反力产生，拱式桥有水平反力产生
\end{answerbox}
\end{questionbox}
...
\end{choices}
\begin{answerbox}
\textbf{答案：} D. 永久作用、可变作用、偶然作用、地震作用
\end{answerbox}
\end{questionbox}
...
\end{choices}
\begin{answerbox}
\textbf{答案：} A. 梁桥、拱桥、刚构桥、缆索承重桥以及组合体系桥
\end{answerbox}
\end{questionbox}
...
\end{choices}
\begin{answerbox}
\textbf{答案：} C. 计算简支梁跨内各截面的最大弯矩时，可按不变化的mc计算；
\end{answerbox}
\end{questionbox}
...
\end{choices}
\begin{answerbox}
\textbf{答案：} B. 将横梁(或桥面板)视为两端搁置在主梁上的简支梁
\end{answerbox}
\end{questionbox}
...
\end{choices}
\begin{answerbox}
\textbf{答案：} B. 板式橡胶支座可以通过变形来适应桥梁的位移和转动。
\end{answerbox}
\end{questionbox}

...
为使数值模拟能够与前述理论计算形成闭环，最终车身模型按以下三个步骤进行复核：
\begin{enumerate}[label=\arabic*.,leftmargin=2.8em]
  \item \textbf{建立车身模型。} 在 MIDAS 中按实物几何关系建立双纵向主梁、横向小杆、轮轴支承及牵引连接杆件的梁单元模型，并输入竹材弹性模量、截面尺寸和壁厚等参数。
  \item \textbf{设置边界与荷载。} 在车轮轴承座位置施加与实际支承相符的约束，在承托区施加满载 $40$ 袋对应的竖向荷载，并将动力放大系数 $1.5$ 纳入设计工况；牵引连接处按小车连接方向保留必要的约束和传力关系。
  \item \textbf{求解并核对结果。} 运行线弹性静力分析，提取车身杆件内力、变形和颜色云图，重点检查双主梁是否形成连续传力路径、轴承座和牵引连接附近是否出现局部突变，并与主梁手算应力 $14.56\MPa$、跨中挠度 $6.21\mm$ 进行数量级和趋势核对。
\end{enumerate}
该流程的作用是把“几何模型、荷载边界和计算结果”三部分明确连接起来。由于当前 MIDAS 截图未显示结果类型、单位和荷载组合，图示结果用于传力路径和相对分布的核对；最终承载结论仍以统一单位下的理论验算、实测尺寸复核和加载试验为准。

\subsubsection{MIDAS 车身模型内力复核}
在方案 C 完成深化后，进一步采用 MIDAS 建立车身梁单元模型，对最终车架的整体传力状态进行数值复核。模型保留两根纵向主梁、横向小杆以及轮轴和牵引连接附近的关键杆件；车轮支承和牵引连接按实际受力位置设置，布袋荷载通过承托区传递到车身主梁。图\ref{fig:midas-body-force}给出了 MIDAS 后处理得到的车身内力分布结果，其中图\ref{fig:midas-body-force-labeled}保留了软件的数值标注，图\ref{fig:midas-body-force-contour}用于观察整体色带变化。

图中颜色用于比较同一结果量在不同杆件位置的相对大小，不将截图中的数值直接与前述手算弯曲应力（$\MPa$）等量比较，因为当前导出的图像未包含结果类型、单位和荷载组合说明。定性上看，两根纵向主梁承担了车身的主要传力，横向小杆主要起荷载分配和联系作用；主梁在靠近轮轴座及牵引连接的区域出现较明显的颜色突变，应将这些位置作为局部加厚、胶接质量和节点对中的重点控制部位。该结果与手算中“主梁为主要承重构件、轴承座区域需局部加强”的判断一致，补足了方案比选之后对最终车身模型的数值模拟验证步骤。

\begin{figure}[H]
\centering
\begin{subfigure}{0.98\linewidth}
  \centering
  \includegraphics[width=\linewidth,trim=8 8 8 8,clip]{images/midas_body_force_labeled.png}
  \caption{保留 MIDAS 数值标注的车身内力结果}
  \label{fig:midas-body-force-labeled}
\end{subfigure}
\vspace{0.35em}
\begin{subfigure}{0.98\linewidth}
  \centering
  \includegraphics[width=\linewidth,trim=8 8 8 8,clip]{images/midas_body_force_contour.png}
  \caption{车身内力结果整体云图}
  \label{fig:midas-body-force-contour}
\end{subfigure}
\caption{MIDAS 车身模型内力分布结果}
...
\subsection{MIDAS轮轴模型内力验证}
为进一步核对轮轴行走模块的受力路径，在 MIDAS 轮轴模型中保留左右车轮、车轴、轴承座和车架连接位置，并采用满载设计反力 $R_{wd}=148.53\Nunit$ 进行静力分析。计算重点为检查车轴的连续传力、轴承座附近的局部响应以及车轮连接区域的内力变化，结果如图\ref{fig:midas-wheel-force}所示。

\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=1.15,x=1cm,y=1cm,every node/.style={font=\small},>=Latex]
  \fill[gray!4] (0.15,0.25) rectangle (12.45,5.05);
  \node[font=\bfseries] at (6.3,4.68){轮轴模型内力云图};

  % 车轮彩色环带
  \foreach \cx in {2.10,10.50}{
    \foreach \a/\b/\c in {0/48/red!82!black,48/92/orange!88!black,92/140/yellow!80!black,140/205/green!72!black,205/270/cyan!82!black,270/324/blue!78!black,324/360/green!72!black}{
      \draw[\c,line width=4.2pt] (\cx,2.45) ++(\a:1.14) arc[start angle=\a,end angle=\b,radius=1.14];
    }
    \draw[fill=white,draw=black!75,line width=0.8pt] (\cx,2.45) circle (0.54);
    \draw[fill=gray!30,draw=black!70] (\cx,2.45) circle (0.24);
    \draw[fill=white,draw=black!70] (\cx,2.45) circle (0.10);
  }

  % 车轴彩色分段
  \draw[black!70,line width=0.8pt] (2.10,2.29) rectangle (10.50,2.61);
  \foreach \x/\c in {2.10/green!72!black,2.80/cyan!82!black,3.50/blue!78!black,4.20/cyan!82!black,4.90/green!72!black,5.60/yellow!80!black,6.30/orange!88!black,7.00/red!82!black,7.70/orange!88!black,8.40/yellow!80!black,9.10/green!72!black,9.80/cyan!82!black}{
    \fill[\c] (\x,2.30) rectangle +(0.72,0.30);
  }

  % 轴承座与车架连接
  \foreach \x in {3.32,9.28}{
    \draw[fill=gray!25,draw=black!65,line width=0.75pt] (\x,2.67) rectangle +(0.72,0.48);
    \draw[fill=white,draw=black!65] (\x+0.36,2.45) circle (0.19);
    \draw[-Latex,green!55!black,line width=0.85pt] (\x+0.36,3.80)--(\x+0.36,3.22);
    \node[above,font=\scriptsize] at (\x+0.36,3.80){支承约束};
  }

  % 设计反力和关键位置
  \draw[-Latex,red!75!black,line width=1.0pt] (2.10,4.08)--(2.10,3.64);
  \draw[-Latex,red!75!black,line width=1.0pt] (10.50,4.08)--(10.50,3.64);
  \node[above,font=\scriptsize,text=red!75!black] at (2.10,4.08){$R_{wd}$};
  \node[above,font=\scriptsize,text=red!75!black] at (10.50,4.08){$R_{wd}$};
  \draw[redmark,rounded corners=0.6mm] (8.84,1.78) rectangle (10.10,3.12);
  \node[below right,font=\scriptsize,text=red!75!black,align=left] at (10.12,2.12){车轮-车轴\\连接区};

  % MIDAS 风格图例
  \node[anchor=west,font=\scriptsize] at (0.60,0.70){相对内力};
  \foreach \x/\c in {2.20/blue!78!black,2.62/cyan!82!black,3.04/green!72!black,3.46/yellow!80!black,3.88/orange!88!black,4.30/red!82!black}{
    \fill[\c] (\x,0.56) rectangle +(0.42,0.26);
  }
  \node[anchor=west,font=\scriptsize] at (4.88,0.69){较小};
  \node[anchor=west,font=\scriptsize] at (6.05,0.69){较大};
  \draw[-Latex,black!55,line width=0.5pt] (5.48,0.69)--(5.92,0.69);
\end{tikzpicture}
\caption{MIDAS 轮轴模型内力分布结果}
\label{fig:midas-wheel-force}
\end{figure}

图中彩色带表示同一计算结果在轮轴各部位的相对变化。车轴中部和轴承座相邻区域承担主要传力，车轮与车轴的连接位置为需要重点复核的部位。该结果与前述轮轴反力计算和轴承座局部承压验算相互对应，因此制作时应优先保证左右轴承座同高、车轴安装平直，并避免轴卡夹紧造成附加阻力。
